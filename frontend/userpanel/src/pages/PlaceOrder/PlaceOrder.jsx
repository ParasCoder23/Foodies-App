...
const PlaceOrder = () => {
    ...
    const onChangeHandler = (event) => {
        ...
        // If state changes, clear the city selection
        if (name === 'state') {
            setData(d => ({ ...d, state: value, city: '' }));
        } else {
            setData(data => ({ ...data, [name]: value }));
        }
    }

    // State -> Cities mapping for cascading selects
    const stateCityMap = {
        'Haryana': ['Ambala', 'Yamunanagar'],
        'Delhi': ['New Delhi'],
        'Punjab': ['Amritsar', 'Patiala', 'Ludhiana'],
        'Himachal Pradesh': ['Shimla', 'Solan'],
        'Karnataka': ['Bengaluru']
    };

    const stateOptions = Object.keys(stateCityMap);
    const cityOptions = data.state ? stateCityMap[data.state] || [] : [];
    ...
    const onSubmitHandler = async (event) => {
        ...
        try {
            ...
            const response = await axios.post(`${API_BASE}/api/orders/create`, orderData, { headers: { 'Authorization': `Bearer ${token}` } });
    ...
    const verifyPayment = async (razorpayResponse) => {
        ...
        try {
            const API_BASE = import.meta.env.VITE_API_URL || 'http://localhost:8080';
            const response = await axios.post(`${API_BASE}/api/orders/verify`, paymentData, { headers: { 'Authorization': `Bearer ${token}` } });
    ...
    const deleteOrder = async (orderId) => {
        try {
            const API_BASE = import.meta.env.VITE_API_URL || 'http://localhost:8080';
            await axios.delete(`${API_BASE}/api/orders/${orderId}`, { headers: { 'Authorization': `Bearer ${token}` } });
        } catch (error) {
    ...
    const clearCart = async () => {
        try {
            const API_BASE = import.meta.env.VITE_API_URL || 'http://localhost:8080';
            await axios.delete(`${API_BASE}/api/cart` , { headers: { 'Authorization': `Bearer ${token}` } });
            setQuantites({});
        } catch (error) {
                ...
                <div className="row g-5"> <div className="col-md-5 col-lg-4 order-md-last"> <h4 className="d-flex justify-content-between align-items-center mb-3"> <span className="text-primary">Your cart</span> <span className="badge bg-primary rounded-pill">{cartItems.length}</span> </h4> <ul className="list-group mb-3">
                    {
                        ...
                        <div className="col-12"> <label htmlFor="address" className="form-label">Address</label> <input type="text" className="form-control" id="address" placeholder="1234 Main St" required name="address" value={data.address} onChange={onChangeHandler} /> </div>
                        <div className="col-md-5"> <label htmlFor="state" className="form-label">State</label> <select className="form-select" id="state" required name="state" value={data.state} onChange={onChangeHandler}> <option value="" disabled>Choose...</option>
                            {stateOptions.map((st) => (
                                <option key={st} value={st}>{st}</option>
                            ))}
                            </select> </div> <div className="col-md-4"> <label htmlFor="city" className="form-label">City</label> <select className="form-select" id="city" required name="city" value={data.city} onChange={onChangeHandler} disabled={!data.state}> <option value="" disabled>Choose...</option>
                                {cityOptions.map((ct) => (
                                    <option key={ct} value={ct}>{ct}</option>
                                ))}
                                </select>
                                {!data.state && <div className="text-danger small mt-1">Please select a state before choosing a city.</div>}
                                </div> <div className="col-md-3"> <label htmlFor="zip" className="form-label">Zip</label> <input type="number" className="form-control" id="zip" placeholder="763812" required name="zip" value={data.zip} onChange={onChangeHandler} /> </div> </div> <hr className="my-4" /> <button className="w-100 btn btn-primary btn-lg" type="submit" disabled={cartItems.length === 0}>Continue to checkout</button> </form> </div>
                </div>
            </main>
        </div>
    )
