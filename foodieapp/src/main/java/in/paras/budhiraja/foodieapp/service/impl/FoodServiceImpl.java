...
public class FoodServiceImpl implements FoodService{
    ...
    @Override
    public FoodResponse addFoodWithImageUrl(FoodRequest request) {
        FoodEntity newFoodEntity = convertToEntity(request);
        newFoodEntity.setImageUrl(request.getImageUrl());
        newFoodEntity = foodRepository.save(newFoodEntity);
        return convertToResponse(newFoodEntity);
    }
    ...
    public void deleteFood(String id) {
        ...
        // Try to delete the image from Cloudinary (but don't let it block DB deletion)
        if(imageUrl != null && !imageUrl.isBlank() && imageUrl.contains("/")){
            try{
                ...
                System.err.println("Failed to delete image from Cloudinary: " + ex.getMessage());
            }
        }
        
        // Always delete from database regardless of image deletion result
